        ...
        with con:
            ...
            if len(data) > 0:
                return json.dumps({'id':data[0][0],'biography':data[0][1],'profilephoto':data[0][2],
                                   ...
                                   'plsgt':data[0][6],'stmgt':data[0][7], 'coverphoto':data[0][8]})
            return None
    except lite.Error:
        return None
    finally:
...
def update_user_prof(database,updated_user_prof):
    ...
    try:
        ...
        with con:
            con.rollback()
            cur = con.cursor()
            cur.execute("UPDATE userprofile SET biography = ?, profilephoto = ?, consoles = ?, genres = ?, xbxgt = ?, plsgt = ?, stmgt = ?, coverphoto = ? WHERE ID = ?;",
                        (updated_user_prof['biography'], updated_user_prof['profilephoto'],
                         ...
                         updated_user_prof['plsgt'],updated_user_prof['stmgt'], updated_user_prof['coverphoto'],
                         updated_user_prof['id']))
            con.commit()
        return True
    except lite.Error:
        return False
